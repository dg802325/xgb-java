 ...
 */
public class OldEntityServiceGenerator extends GeneratorConfig{
    ...
    public static Boolean genGeratorCode(String path,String tableName,List<TableColumnInfo> tableColumnInfos) {
        ...
        data.put("codeEntity1",codeEntity1(tableColumnInfos));
        data.put("codeEntity2",codeEntity2(tableColumnInfos));
        try {
            File controllerFile = new File(path  +SLASH+ "Save"+StringUtils.tableNameConvertUpperCamel(tableName)+"Service.java");
            if (!controllerFile.getParentFile().exists()) {
                controllerFile.getParentFile().mkdirs();
            }
            cfg.getTemplate("oldEntityService.ftl").process(data, new FileWriter(controllerFile));
        } catch (Exception e) {
