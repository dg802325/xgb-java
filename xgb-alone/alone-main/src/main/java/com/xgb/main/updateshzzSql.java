...
public class updateshzzSql {
    ...
    public static void getSql(List<Shzz> shzzes){
        ...
        for (Shzz shzz : shzzes) {
            ...
            sbf.append("update jc_shzz_detail set zgmc = '").append(shzz.getNewZgmc()).append("' where title = '").append(shzz.getName()).append("';");
...
//                jj.setId(row.getCell(0).getStringCellValue());
                jj.setName(row.getCell(1).getStringCellValue());
                jj.setZgmc(row.getCell(2).getStringCellValue());
                jj.setNewZgmc(row.getCell(3).getStringCellValue());
                list.add(jj);
            }
        }
        return list;
