...
public class SaveMjzzUserService {
    ...
    public static StringBuffer saveEntity(StringBuffer buffer , List<MjzzUser> contents){
        ...
        Integer managerdept = content.getManagerdept ();//登记管理机关
        ...
        Integer type = content.getType ();//类型1,社团,2,民非3基金会,4.拟成立境外基金会分支机构名称,5 国际社团6 外国商会7 涉外基金会,8 登记机关用户
        Integer status = content.getStatus ();// 用户状态,1,已批准申请人用户，2未批准申请人用户，3，社会组织用户0，已无效用户
        Integer typedeptid = content.getTypedeptid ();//用户类型1,自然人3单位,2,公募基金会,4非公募基金会,5境外基金会分支代表机构
        Integer manageorg = content.getManageorg ();//主管单位
        String organizationId = content.getOrganizationId ();//拟成立基金会类型1公募，2非公募，拟成立境外基金会分支机构名称
        Integer mfType = content.getMfType ();//用户证件类型1,身份证,2,军官证,3,护照
        Integer region = content.getRegion ();//null
        Date registedate = content.getRegistedate ();//注册日期
        Integer area = content.getArea ();//公募基金会募捐地域1全国，2地方
        Integer usertype = content.getUsertype ();//拟成立类型1基金会，2境外基金会分支/代表机构
        ...
        Integer logginginCount = content.getLogginginCount ();//登陆次数
