            ...
            <tr>
              <td align="left">
                <div class="t-sku-info">
                  ...
                  <div class="info">
                    <p class="name">
                      <router-link :to="'/detail/'+sku.id">{{sku.skuName}}</router-link>
                    </p>
        ...
        <div>
          ...
          <div class="order-summary">
            ...
            <p>
              ...
              <span class="value">¥{{sku.skuExpressPrice? sku.skuExpressPrice: 0}}</span>
            </p>
            <p>
              <span class="label">商品总价：</span>
              <span class="value">¥{{skuSumSparce}}</span>
            </p>
...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      const {skuId, skuSum} = this.$route.params;
      if (!skuId) return;
      if (!token) {
    ...
    methods: {
      async getSku(params) {
        let res = await this.$get('/api/getSku', params);
        if (res.code === '200') {
      ...
      async handleSubmit() {
        ...
        let res = await this.$post('/api/settlement',params);
        if(res.code==='200'){
