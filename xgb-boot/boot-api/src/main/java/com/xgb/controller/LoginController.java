...
public class LoginController {
    ...
    public R login(SysUser sysUser, HttpServletResponse response,String code, HttpServletRequest request){
        ...
        jToken.setUserType("boot-backend");
        try {
            Subject subject = org.apache.shiro.SecurityUtils.getSubject();
            subject.login(jToken);
            Object adminToken = org.apache.shiro.SecurityUtils.getSubject().getPrincipal();
            response.addHeader("boot-backend", adminToken.toString());
            CookieUtils.setCookie(response, "boot-backend", adminToken.toString(), tokenTime);
            Map map = new HashMap();
            map.put("boot-backend", adminToken);
