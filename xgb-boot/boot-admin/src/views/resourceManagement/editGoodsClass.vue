<template>
    <div>
        <glob-breadcrumb title="编辑物资分类"/>
        <br><br>
        <div class="z-di-cc">
            <div class="z-space-cc">
                <el-steps :space="340" :active="1" finish-status="success" align-center>
                    <el-step title="编辑物资分类"></el-step>
                    <el-step title="编辑物资信息"></el-step>
                </el-steps>
            </div>
        </div>
        <!--内容-->
            ...
            <div class="content_r">
                <div class="center_r_t">
                    ...
                    <p style="padding-top:25px;min-height:16px;padding-left:115px;color: rgb(153, 153, 153);">
                        <span v-if="oneCategoryId">您当前选择的物资类别是：<span style="color: rgb(240, 72, 68);">{{oneCategoryId | formatData(jsonMap)}} > {{twoCategoryId | formatData(jsonMap)}} > {{threeCategoryId | formatData(jsonMap)}} </span></span>
                    </p>
                    <div class="c-bottom">
                    </div>
                    <div>
                        <el-button type="primary" size="15" @click="toWriteSkuInfo"
                                   style="margin-top: 35px;margin-left: 285px;width: 200px;background-color: #33d0b2">下一步, 编辑物资信息
                        </el-button>
                    </div>
                    <br/>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            async toWriteSkuInfo() {
                ...
                // this.$router.push('/stock/resourceManagement/addGoodInformation' + '/'+ this.threeCategoryId)
                // this.$router.push('/editResource/'+this.parentId + '/'+ this.threeCategoryId)
                this.$router.push('/stock/resourceManagement/editResource/'+this.threeCategoryId + '/'+ this.spuId)
                // this.$router.push('/stock/resourceManagement/addGoodInformation' + '/'+ this.threeCategoryId)
