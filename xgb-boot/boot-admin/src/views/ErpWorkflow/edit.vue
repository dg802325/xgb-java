                                ...
                                <el-table
                                    ...
                                    <el-table-column prop="erpDepartment" align="center" label="部门/岗位" width="200px;">
                                        <template slot-scope="{row}">
                                            <!--                                    <span v-if="row">{{row.departmentName}}/{{row.roleName}}</span>-->
...
<!--                        <div style="display:flex;">-->
                        <el-form-item label="节点序号:" required>
                            ...
                            <!--                                    <el-input v-model="nodeProcess" size="1" style="width: 200px;"></el-input>-->
                            <!--                                </el-form-item>-->
    ...
    export default {
        ...
        methods: {
            ...
            async saveErpStorehouse() {
                ...
                let res = await this.$post("/supplier/getErpWorkflowNodeForPage", data);
                if (res.code == 200) {
                    this.$message({
                        showClose: true,
                        message: res.message,
            ...
            //回显数据
            async backData(id){
                let res = await this.$get('/admin/view_erpWorkflow',{id:id})
                if (res.code==200) {
                    this.editItem = res.item;
                    this.wfNumber = this.editItem.wfNumber
                    this.wfName = this.editItem.wfName
                    this.wfRemark = this.editItem.wfRemark
                }
            },
            ...
            // async delExpert() {
            //     let res = await this.$post("admin/remove_erpWorkflowNode", {id: this.item.id})
            //     if (res.code == 200) {
