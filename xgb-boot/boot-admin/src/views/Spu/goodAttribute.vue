...
<!--                </div>-->
                ...
                <div style="padding-top: 50px;padding-left: 250px">
                    <button @click="retreat" style="width: 180px;height: 35px;border: 1px solid #1abcb6;color: #1abcb6;background: white">上一步，填写物资信息</button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async saveGoods(){
                let resa = await this.$get("/admin/getSpuShop", {id:this.parentId});
                let data = {
                    spuId: this.spuId,
                ...
                let res = await this.$post("/admin/insertSpuResources", data);

                let data1 = {
                    id:this.spuId,
                    spuExplain:this.spuExplain
                };
                let resb = await this.$post("/admin/insertSpu", data1);
                this.$router.push('/stock/resourceManagement')

            },
            async retreat(){
                // this.$router.push('/AddShopsAttribute/ReturnInformation/' + this.skuId+'/'+this.parentId)
                this.$router.push('/stock/resourceManagement/returnClassAttribute/'+this.parentId + '/'+this.spuId)
