...
import com.xgb.model.JcContentExt;
    ...
    public static JcContentExt save(JcContentExt jcContentExt){
        ...
        try {
            ...
            if(MyTools.isEmpty(jcContentExt.getToplevelDate())){
                ...
                ps.setDate(19,new java.sql.Date(jcContentExt.getToplevelDate().getTime()));
            }
            if(MyTools.isEmpty(jcContentExt.getPigeonholeDate())){
                ps.setNull(20,Types.DATE);
            }else {
                ps.setDate(20,new java.sql.Date(jcContentExt.getPigeonholeDate().getTime()));
            }
            ps.setDate(21,new java.sql.Date(jcContentExt.getReleaseDate().getTime()));
            ps.executeUpdate();
            conn.commit();
