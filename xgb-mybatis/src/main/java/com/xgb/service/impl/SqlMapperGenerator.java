...
public class SqlMapperGenerator extends CodeGeneratorManager implements CodeGenerator {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(SqlMapperGenerator.class);
    ...
    public void genCode(String tableName) {
        ...
        try {
            ...
            File controllerFile = new File(PROJECT_PATH + PACKAGE_MAPPER_SQL_XML_PATH + customMapping
                    + modelNameUpperCamel + "SqlMapper.java");
            if (!controllerFile.getParentFile().exists()) {
                controllerFile.getParentFile().mkdirs();
            }
            cfg.getTemplate("sqlMapper.ftl").process(data, new FileWriter(controllerFile));
            logger.info(modelNameUpperCamel + "sqlMapper.java 生成成功!");
        } catch (Exception e) {
            throw new RuntimeException("sqlMapper.java 生成失败!", e);
        }
    ...
    private List<String> getSqlMapperXmlModel(String tableName, String modelName,
        ...
        org.mybatis.generator.config.Configuration cfg = new org.mybatis.generator.config.Configuration();
